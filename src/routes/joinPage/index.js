...
const JoinPage = () => {
    ...
    return (<div className={style.JoinPage}>
        <div className={style.JoinBox}>
            ...
            <OutlineInput className={style.JoinInput} value={password} onInput={setUserPw} type='password' placeholder='패스워드' />
            ...
            <OutlineButton onClick={async () => {
                if (!phoneNumber.length || !name.length || !password.length || !checkPw.length) {
                    alert('항목을 입력해주세요');
                    return;
                }
